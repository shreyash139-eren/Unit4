    ...
    return (
        <div className="grid justify-center items-center h-[100vh] border-1 w-[100%] bg-white-200">
            <div className="grid h-[350px] rounded-3xl justify-center bg-red-800 p-10">
                <p className="text-2xl font-bold text-center text-white">Signup</p>
                <form onSubmit={handleSubmit}>
                    ...
                    <button className="bg-blue-400 mt-2 text-white px-5 py-1 text-lg rounded-full hover:bg-blue-500 cursor-pointer" disabled={loading}>{loading?"signing up...":"signup"}</button>
                </form>
                <Link to={"/login"} className="text-lg text-center text-white hover:text-green-600">Already have an account? Login</Link>
            </div>
        </div>
    )
}
